...
import ratio.automate.base.github.model.GitHubLabelName
...
class DetermineActionTest {
    ...
    val ratioBot = GitHubUser(GitHubUsername(Constants.RATIO_BOT_USERNAME))
    ...
    fun `happy path -AssignIssue - multiple comments`() = testScope {
        ...
        coEvery {
            ...
            GitHubComment(ratioBot, "Random message"),
    ...
    @Test
    fun `happy path - DoNothing - ratio bot comment`() = testScope {
        // given
        coEvery {
            gitHubService.fetchIssueComments(issueNumber)
        } returns listOf(
            GitHubComment(user1, "I'm on it"),
            GitHubComment(user2, "I'm on it"),
            GitHubComment(ratioBot, "Okay, I'm on it"),
        ).right()
        coEvery {
            gitHubService.fetchIssue(issueNumber)
        } returns newIssue().right()
