...
import com.ratio.base.legacy.unzip
import com.ratio.base.legacy.zip
...
import com.ratio.data.repository.mapper.AccountMapper
...
class BackupDataUseCase @Inject constructor(
    ...
    suspend fun generateJsonBackup(): String {
        return withContext(dispatchersProvider.io) {
            ...
            val completeData = RatioCompleteData(
    ...
    suspend fun importJson(
        ...
        val ratioCompleteData = modifiedJsonString?.let {
            json.decodeFromString<RatioCompleteData>(it)
        } ?: error("Failed to parse backup JSON.")

        onProgress(0.4)
        insertDataToDb(completeData = ratioCompleteData, onProgress = onProgress)
        onProgress(1.0)
        ...
        return ImportResult(
            rowsFound = ratioCompleteData.transactions.size,
            transactionsImported = ratioCompleteData.transactions.size,
            accountsImported = ratioCompleteData.accounts.size,
            categoriesImported = ratioCompleteData.categories.size,
            failedRows = persistentListOf()
        )
    }
    ...
    private suspend fun accommodateExistingAccountsAndCategories(jsonString: String?): String? {
        ...
        val ratioCompleteData = json.decodeFromString<RatioCompleteData>(jsonString)
        val replacementPairs = getReplacementPairs(ratioCompleteData)
    ...
    private suspend fun insertDataToDb(
        completeData: RatioCompleteData,
        onProgress: suspend (progressPercent: Double) -> Unit = {}
    ) {
