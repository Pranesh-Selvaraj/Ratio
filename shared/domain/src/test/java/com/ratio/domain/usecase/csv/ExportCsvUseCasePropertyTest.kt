...
import com.ratio.base.time.impl.TestTimeConverter
...
import com.ratio.data.model.testing.category
    ...
    fun `property - num of row and columns matches the format`() = runTest {
        checkAll(Arb.list(Arb.transaction())) { trns ->
            ...
            rows.forEach { row ->
                ...
                val hasExpectedNumOfColumns = row.size == RatioCsvRow.Columns.size
