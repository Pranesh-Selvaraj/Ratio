...
import com.ratio.data.model.primitive.toNonNegative
...
class ExportCsvUseCase @Inject constructor(
    ...
    suspend fun exportCsv(
        ...
        buildString {
            append(RatioCsvRow.Columns.joinToString(separator = CSV_SEPARATOR))
            append(NEWLINE)
            for (trn in transactions) {
                append(
                    trn.toRatioCsvRow().toCsvString(
    ...
    private fun RatioCsvRow.toCsvString(
    ...
    private fun Transaction.toRatioCsvRow(): RatioCsvRow = when (this) {
        is Expense -> expenseCsvRow()
        is Income -> incomeCsvRow()
        is Transfer -> transferCsvRow()
    }

    private fun Expense.expenseCsvRow(): RatioCsvRow = RatioCsvRow(
        date = time.takeIf
        { settled },
        title = title,
        category = category,
    ...
    private fun Income.incomeCsvRow(): RatioCsvRow = RatioCsvRow(
    ...
    private fun Transfer.transferCsvRow(): RatioCsvRow = RatioCsvRow(
        date = time.takeIf { settled },
        title = title,
        category = category,
        account = fromAccount,
