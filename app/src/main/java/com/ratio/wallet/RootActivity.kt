...
import com.ratio.legacy.utils.activityForResultLauncher
import com.ratio.legacy.utils.simpleActivityForResultLauncher
...
import com.ratio.wallet.ui.applocked.AppLockedScreen
import com.ratio.widget.balance.WalletBalanceWidgetReceiver
import com.ratio.widget.transaction.AddTransactionWidget
...
class RootActivity : AppCompatActivity(), RootScreen {
    @Inject
    lateinit var ratioContext: RatioCtx
        ...
        setContent {
            ...
            RatioMaterial3Theme(
                dark = isDarkThemeEnabled(
                    ratioDesign = appDesign(ratioContext),
                    systemDarkTheme = isSystemInDarkTheme
                ),
                isTrueBlack = appDesign(ratioContext).context().theme == Theme.AMOLED_DARK
            ) {
    ...
    private fun setupDatePicker() {
        ratioContext.onShowDatePicker = { minDate,
                                        maxDate,
                                        initialDate,
                                        onDatePicked ->
            val datePicker =
    ...
    private fun setupTimePicker() {
        ratioContext.onShowTimePicker = { initialTime,
                                        onTimePicked ->
    ...
    private fun isDarkThemeEnabled(ratioDesign: RatioDesign, systemDarkTheme: Boolean): Boolean {
        return when (ratioDesign.context().theme) {
            Theme.LIGHT -> false
            Theme.DARK -> true
    ...
    private fun createFileLauncher() {
        ...
        ratioContext.createNewFile = { fileName, onFileCreatedCallback ->
    ...
    private fun openFileLauncher() {
        ...
        ratioContext.openFile = { onFileOpenedCallback ->
