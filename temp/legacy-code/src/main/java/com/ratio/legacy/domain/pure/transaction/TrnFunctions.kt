...
import com.ratio.wallet.domain.pure.account.accountCurrency
...
object LegacyTrnFunctions {
    @Pure
    fun expenses(transactions: List<com.ratio.base.legacy.Transaction>): List<com.ratio.base.legacy.Transaction> {
        return transactions.filter { it.type == TransactionType.EXPENSE }
    }

    @Pure
    fun incomes(transactions: List<com.ratio.base.legacy.Transaction>): List<com.ratio.base.legacy.Transaction> {
        return transactions.filter { it.type == TransactionType.INCOME }
    }

    @Pure
    fun trnCurrency(
        transaction: com.ratio.base.legacy.Transaction,
        accounts: List<Account>,
        baseCurrency: String
    ): Option<String> {
