...
fun Double.format(currencyCode: String): String {
    return this.format(RatioCurrency.fromCode(currencyCode))
}

fun Double.format(currency: RatioCurrency?): String {
...
fun decimalPartFormatted(currency: String, value: Double): String {
    return if (RatioCurrency.fromCode(currency)?.isCrypto == true) {
...
fun formatInputAmount(
    ...
    val decimalCountOkay = RatioCurrency.fromCode(currency)?.isCrypto == true ||
