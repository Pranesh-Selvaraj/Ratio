package com.ratio.wallet.domain.pure.transaction

import arrow.core.NonEmptyList
import arrow.core.nonEmptyListOf
import com.ratio.data.model.Transaction
import com.ratio.frp.Pure
import com.ratio.wallet.domain.pure.util.mapIndexedNel
import com.ratio.wallet.domain.pure.util.mapIndexedNelSuspend
import com.ratio.wallet.domain.pure.util.nonEmptyListOfZeros
...
object LegacyFoldTransactions {
    ...
    suspend fun <Arg> foldTransactionsSuspend(
        ...
        valueFunctions: NonEmptyList<suspend (com.ratio.base.legacy.Transaction, Arg) -> BigDecimal>,
        arg: Arg
    ): NonEmptyList<BigDecimal> = sumTransactionsSuspendInternal(
        transactions = transactions,
        valueFunctions = valueFunctions,
    ...
    internal tailrec suspend fun <A> sumTransactionsSuspendInternal(
        transactions: List<com.ratio.base.legacy.Transaction>,
        valueFunctionArgument: A,
        valueFunctions: NonEmptyList<suspend (com.ratio.base.legacy.Transaction, A) -> BigDecimal>,
        sum: NonEmptyList<BigDecimal> = nonEmptyListOfZeros(n = valueFunctions.size)
    ): NonEmptyList<BigDecimal> {
        return if (transactions.isEmpty()) {
            sum
    ...
    suspend fun <A> sumTrns(
        transactions: List<com.ratio.base.legacy.Transaction>,
        valueFunction: suspend (com.ratio.base.legacy.Transaction, A) -> BigDecimal,
        argument: A
    ): BigDecimal {
        return sumTransactionsSuspendInternal(
            transactions = transactions,
