...
import com.ratio.design.l0_system.RatioTheme
import com.ratio.ui.time.TimeFormatter

val LocalRatioContext = compositionLocalOf<RatioContext> { error("No LocalRatioContext") }
...
fun RatioUI(
    ...
    val ratioContext = design.context()

    CompositionLocalProvider(
        LocalRatioContext provides ratioContext,
        ...
        RatioTheme(
            theme = ratioContext.theme,
            design = design
        ) {
            WrapWithSurface(includeSurface = includeSurface) {
                BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
                    ratioContext.screenWidth = with(LocalDensity.current) {
                        maxWidth.roundToPx()
                    }
                    ratioContext.screenHeight = with(LocalDensity.current) {
                        maxHeight.roundToPx()
                    }
...
@Deprecated("Old design system. Use `:ratio-design` and Material3")
@Composable
fun ratioContext(): RatioContext {
    return LocalRatioContext.current
}
