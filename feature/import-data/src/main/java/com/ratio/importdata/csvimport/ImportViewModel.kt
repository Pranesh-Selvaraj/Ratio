package com.ratio.importdata.csvimport
...
import com.ratio.legacy.domain.deprecated.logic.csv.CSVImporter
import com.ratio.legacy.domain.deprecated.logic.csv.model.ImportType
import com.ratio.data.backup.BackupDataUseCase
import com.ratio.legacy.utils.asLiveData
import com.ratio.base.legacy.getFileName
...
import com.ratio.wallet.domain.deprecated.logic.csv.CSVMapper
import com.ratio.wallet.domain.deprecated.logic.csv.CSVNormalizer
...
class ImportViewModel @Inject constructor(
    ...
    private suspend fun restoreCSVFile(fileUri: Uri, importType: ImportType): ImportResult {
        return com.ratio.legacy.utils.ioThread {
            val rawCSV = fileReader.read(
                uri = fileUri,
                charset = when (importType) {
                    ImportType.RATIO -> Charsets.UTF_16
                    else -> Charsets.UTF_8
                }
            ).getOrNull()
    ...
    private suspend fun hasCSVExtension(
        context: Context,
        fileUri: Uri
    ): Boolean = com.ratio.legacy.utils.ioThread {
