...
class RatioRepositoryDataSource @Inject constructor(
    ...
    @Keep
    @Serializable
    data class RatioRepositoryInfo(
        @SerialName("forks")
        val forks: Int,
    ...
    private suspend fun getContributorsFromRequest(currentPage: Int): Either<String, List<ContributorDto>> =
            ...
            val contributorsDto = httpClient
                .get("https://api.github.com/repos/Pranesh-Selvaraj/Ratio/contributors") {
    ...
    suspend fun fetchRepositoryInfo(): RatioRepositoryInfo? {
        return try {
            withContext(Dispatchers.IO) {
                httpClient
                    .get("https://api.github.com/repos/Pranesh-Selvaraj/Ratio")
                    .body<RatioRepositoryInfo>()
            }
        } catch (e: Exception) {
            null
        }
