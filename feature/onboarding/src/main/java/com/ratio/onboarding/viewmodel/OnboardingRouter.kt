...
class OnboardingRouter(
    ...
    // ------------------------------------- Step 5 - Categories ------------------------------------
    suspend fun categoriesNext(baseCurrency: RatioCurrency?) {
        completeOnboarding(baseCurrency = baseCurrency)
    }

    suspend fun categoriesSkip(baseCurrency: RatioCurrency?) {
        completeOnboarding(baseCurrency = baseCurrency)
    ...
    private suspend fun completeOnboarding(
        ...
        ioThread {
            ...
            AssetCode.from(baseCurrency?.code ?: RatioCurrency.getDefault().code)
                .onRight {
